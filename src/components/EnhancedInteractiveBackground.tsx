...
export default function EnhancedInteractiveBackground() {
  ...
  useEffect(() => {
    ...
    // Camera setup with wider angle and pulled back
    ...
    // Create a geometric crystal shape - made larger and more prominent
    ...
    // Add outer glow
    const glowGeometry = new THREE.IcosahedronGeometry(1.4, 0);
    ...
    // Add orbiting rings - larger for visibility
    const ring1Geometry = new THREE.TorusGeometry(2.0, 0.03, 16, 100);
    ...
    const ring2Geometry = new THREE.TorusGeometry(2.4, 0.02, 16, 100);
    ...
    // Add orbiting satellite
    const satelliteGeometry = new THREE.TetrahedronGeometry(0.2, 0);
    ...
    interactiveGroup.add(satelliteOrbit);
    
    // Add a second satellite with different color
    const satellite2Geometry = new THREE.OctahedronGeometry(0.15, 0);
    ...
    satellite2.position.set(0, 0, -2.2);
    const satellite2Orbit = new THREE.Group();
    satellite2Orbit.rotation.x = Math.PI / 2;
    satellite2Orbit.add(satellite2);
    interactiveGroup.add(satellite2Orbit);
    ...
    // Position the interactive object to be visible 
    interactiveGroup.position.x = isMobile ? 0 : 4;
    interactiveGroup.position.y = 0;
    interactiveGroup.position.z = 0;
    interactiveGroup.scale.set(1, 1, 1);
    ...
    const animate = () => {
      ...
      satelliteOrbit.rotation.y = elapsedTime * 0.5;
      satellite.rotation.y = elapsedTime * 1.5;
      
      satellite2Orbit.rotation.z = -elapsedTime * 0.3;
      satellite2.rotation.x = elapsedTime * 1.2;
      
      // Scroll effect - move crystal slightly with scrolling
    ...
    return () => {
      ...
      [
        starsGeometry, starsMaterial, crystalGeometry, crystalMaterial,
        glowGeometry, glowMaterial, particlesGeometry, particlesMaterial,
        ring1Geometry, ring1Material, ring2Geometry, ring2Material,
        satelliteGeometry, satelliteMaterial, satellite2Geometry, satellite2Material
      ].forEach(resource => {
  ...
  return (
    <div className="absolute inset-0 -z-10">
      ...
      <div className="absolute inset-0 bg-gradient-to-br from-gray-900 via-gray-800 to-indigo-900 opacity-60"></div>
